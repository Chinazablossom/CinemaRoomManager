...
/*
--Description:
There are many enjoyable activities on this funny little planet Earth,
and still, the happiest, simplest, and most fulfilling one is probably going to the movies.
...
fun main() {
    ...
    val seatSymbol = " S"
    println("Cinema:")
    for (numberOfSeats in 1..seat) {
        print(" $numberOfSeats")
    }
    println()
    for (numberOfRow in 1..row) {
        println("$numberOfRow ${seatSymbol.repeat(seat)}")
    }
}